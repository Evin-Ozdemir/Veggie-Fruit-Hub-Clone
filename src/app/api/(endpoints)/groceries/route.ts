...
// Mock data - Tüm kategoriler ve public klasöründeki resimler
const mockGroceries = [
  // Meyveler
  {
    _id: "1",
...
export async function GET(request: NextRequest) {
  try {
    ...
    let filteredGroceries = [...mockGroceries];
    ...
    if (organic === "true") {
      filteredGroceries = filteredGroceries.filter(
        (item) => item.isOrganic === true
      );
    }

    return NextResponse.json({ groceries: filteredGroceries }, { status: 200 });
  } catch (error) {
    console.error("Error fetching groceries:", error);
    return NextResponse.json(
      { message: "Manav verileri alınamadı" },
      { status: 500 }
    );
  }
}
